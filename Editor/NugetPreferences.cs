namespace NugetForUnity
{
    ...
    public static class NugetPreferences
    {
        ...
        /// <summary>
        /// New temporal repositoryPath
        /// </summary>
        private static string newRepositoryPath;
        ...
        public static void PreferencesGUI()
        {
            ...
            GUILayout.Label("Repository Path(There must be parent foler)", GUILayout.ExpandWidth(false));
            if (string.IsNullOrEmpty(newRepositoryPath)) newRepositoryPath = NugetHelper.NugetConfigFile.SavedRepositoryPath;
            newRepositoryPath = EditorGUILayout.TextField(newRepositoryPath);
            if (newRepositoryPath != NugetHelper.NugetConfigFile.SavedRepositoryPath)
            {
                if (GUILayout.Button("Apply", GUILayout.ExpandWidth(false)))
                {
                    preferencesChangedThisFrame = true;
                    var prevPath = Path.Combine("Assets", NugetHelper.NugetConfigFile.SavedRepositoryPath).Replace("./", "");
                    var newPath = Path.Combine("Assets", newRepositoryPath).Replace("./","");
                    var dirInfo = new DirectoryInfo(newPath);
                    var moveError = AssetDatabase.MoveAsset(prevPath, newPath);
                    if (string.IsNullOrEmpty(moveError))
                    {
                        NugetHelper.NugetConfigFile.SetNewSavedRepositoryPath(newRepositoryPath);
                    }
                    else
                    {
                        Debug.LogError(moveError);
                        newRepositoryPath = NugetHelper.NugetConfigFile.SavedRepositoryPath;
                    }
                }
            }
