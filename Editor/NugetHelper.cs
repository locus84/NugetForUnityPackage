namespace NugetForUnity
{
    ...
    public static class NugetHelper
    {
	    ...
	    private const string SettingDirectory = "UserSettings";
#else
        private const string SettingDirectory = "ProjectSettings";
#endif

        /// <summary>
        /// The path to the nuget.config file.
        /// </summary>
        public static readonly string NugetConfigFilePath = Path.Combine(Application.dataPath, $"../{SettingDirectory}/NuGet.config");

        /// <summary>
        /// The path to the packages.config file.
        /// </summary>
        private static readonly string PackagesConfigFilePath = Path.Combine(Application.dataPath, $"../{SettingDirectory}/NuGetPackages.config");

        /// <summary>
        /// The path to package tools
        /// </summary>
        private static readonly string ToolsPackageFolder = Path.Combine(Application.dataPath, "../Packages");
        ...
        private static void RunNugetProcess(string arguments, bool logOuput = true)
        {
            ...
            Directory.CreateDirectory(ToolsPackageFolder);

            string[] files = Directory.GetFiles(ToolsPackageFolder, "nuget.exe", SearchOption.AllDirectories);
            if (files.Length > 1)
            {
            ...
            else if (files.Length < 1)
            {
                ...
                files = Directory.GetFiles(ToolsPackageFolder, "nuget.exe", SearchOption.AllDirectories);
                if (files.Length < 1)
                {
        ...
        private static CredentialProviderResponse? GetCredentialFromProvider_Uncached(Uri feedUri, bool downloadIfMissing)
        {
            ...
            // Try to find any nuget.exe in the package tools installation location
            possibleCredentialProviderPaths.Add(ToolsPackageFolder);
